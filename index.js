      ...
      if (channelName.indexOf(guildConfig.channelPrefix) != -1 || channelName.indexOf(changeObj.channelPrefix) != -1) {
        ...
        if (checkPerm(guild, "MANAGE_ROLES")) {
          ...
          if (member) {
            ...
            cchannel.overwritePermissions(member.user, p).then(console.log("Applied changes."));
          }
        ...
        switch (cmd) {
          ...
          case "configRole":
            if (newValue == "false") {
              ...
              if (role == null) {
                message.reply(
                  "you need to specifiy an existing role. Please add the role **" + newValue + "** and try again."
                );
              } else {
                changeValid = true;
              }
