      ...
      if (configurationID) {
        ...
        if (configuration) {
          ...
          if (configuration.createTextChannel) {
            let textchannelID = dcg.channels[configurationID].find((c) => c.voiceChannel == oldMember.voiceChannelID)
              .textChannel;
...
*${c.description}*\n`;
  ...
  reply += "\nIf you encounter any issues or have questions, feel free to join my Discord: https://discord.gg/hwrb46E.";
...
function CCprefix(message, args, guild, dcg) {
  ...
  reply += `Current value: **${
    dcg.getConfiguration(dcg.setup.id)[detail].prefix == null ? "No" : dcg.getConfiguration(dcg.setup.id)[detail].prefix
  }**\n`;
...
function CCsuffix(message, args, guild, dcg) {
  ...
  reply += `Current value: **${
    dcg.getConfiguration(dcg.setup.id)[detail].suffix == null ? "No" : dcg.getConfiguration(dcg.setup.id)[detail].suffix
  }**\n`;
...
function CCchannelname(message, args, guild, dcg) {
  ...
  reply += `**5**: Nickname of the owner. Example: *${
    message.member.nickname || `awesome ${message.author.username}`
  }*\n`;
...
function cancelSetup(message, args) {
  ...
  if (dcg.setup.state) {
    message.reply(
      `the current setup of **${dcg.data.configurations.find((c) => (c.id = dcg.setup.id)).name}** has been canceled.`
    );
  } else {
...
function showSettings(message, args) {
  ...
  rfields.push({
    name: "**Bot State**",
    value: `*${DynChanGuilds[guild.id].data.toggle.substring(0, 1).toUpperCase()}${DynChanGuilds[
      guild.id
    ].data.toggle.substring(1)}*`,
  });
  rfields.push({
    name: "**Custom Prefix**",
  ...
  let ereply = {
    ...
    footer: {
      text: `Use ${botPrefix} help to see all commands. If you encounter any issues or have questions, feel free to join my Discord: https://discord.gg/hwrb46E.`,
    },
    thumbnail: {
    ...
    DynChanGuilds[guild.id].data.configurations.forEach((c) => {
      ...
      let ereply = {
        ...
        thumbnail: {
          ...
          }&size=128&background=${translateHex(c.color)}&color=FFFFFF&rounded=true&bold=true`,
        },
        fields: rfields,
      };
      message.channel.send(`Configuration ${count} of ${DynChanGuilds[guild.id].data.configurations.length}`, {
        embed: ereply,
      });
    });
  } else
